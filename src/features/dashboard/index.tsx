...
export default function Dashboard() {
  ...
  const [articlesReports, setArticlesReports] =
    useState<ReportArticleStatistics>()
  const [articlesThematic, setArticlesThematic] =
    useState<ReportArticleThematics>()
  ...
  useEffect(() => {
    void reportsService
      .getArticlesReports()
      .then((response: ReportArticleStatistics) => {
  ...
  useEffect(() => {
    void reportsService
      .getArticlesThematics()
      .then((response: ReportArticleThematics) => {
    ...
    <section className="bg-gray-50 p-6 rounded-lg">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard
          ...
          value={articlesReports?.totalArticles ?? 0}
          icon={FileText}
          iconColor="text-blue-500"
        />
        <StatCard
          title="Vistas Totales"
          value={articlesReports?.totalViews ?? 0}
          icon={Eye}
          iconColor="text-green-500"
        />
        <StatCard
          title="Descargas Totales"
          value={articlesReports?.totalDownloads ?? 0}
          icon={Download}
