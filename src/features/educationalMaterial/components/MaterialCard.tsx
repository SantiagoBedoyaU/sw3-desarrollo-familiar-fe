...
import EducationalMaterial, { EducationalMaterialType } from '../entities/EducationalMaterial'
...
const MaterialCard = ({ material, setUpdatedMaterials }: MaterialCardProps) => {
  ...
  const canEditDelete = userRole === ADMIN_ROLE || userRole === TEACHER_ROLE
  ...
  const getIcon = () => {
    if (material.type === EducationalMaterialType.Document) return (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
      </svg>
    )
    if (material.type === EducationalMaterialType.Image) return (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
    )
    if (material.type === EducationalMaterialType.Resource) return (
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <span className="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded">
            ...
            {material.type === EducationalMaterialType.Resource && 'Recurso Web'}
            {material.type === EducationalMaterialType.Other && 'Otro'}
          </span>

          {material.minAge !== undefined && material.maxAge !== undefined && (
            <span className="inline-block bg-green-100 text-green-800 text-xs px-2 py-1 rounded">
              {material.minAge.toString() === material.maxAge.toString()
                ? `${material.minAge.toString()} años`
                : `${material.minAge.toString()}-${material.maxAge.toString()} años`}
            </span>
          )}
        </div>
